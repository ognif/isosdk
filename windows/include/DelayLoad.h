/*****************************************************************************
 * delayload.h: Class to make working with external dlls easier
 *****************************************************************************
 * Copyright (C) 2004-2005 ShrinkTo5
 *
 * Authors: Ocrana		 <info@shrinkto5.com>
 *          Andrej Grecu <info@shrinkto5.com>
 *
...
#define DL_REPEAT_PARAM_IMPL_N1(n, m, d1, d2)	DL_CAT(n,1)d2() DL_CAT(m,1)
...
#define DL_DECLARE_NAME_ID(id, name)	DL_DECLARE_NAME_ID_IMPL(id, name, LPCTSTR,_T)
	...
	{
#ifdef VC6
		CLWMutex(LONG &pFlag):m_pFlag(pFlag)
		{
		}
#else
